...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:test@localhost/room_reservations'
...
def register():
    ...
    user = User(user_name=data['user_name'], password=generate_password_hash(data['password']), email=data['email'])
    db.session.add(user)
    db.session.commit()
...
def reserve():
    ...
    if data['occupancy'] > room.capacity:
        return jsonify({'message': 'Requested occupancy is greater than room capacity'}), 400
    ...
    for i in range(4):
        date = datetime.datetime.strptime(data['date'], '%Y-%m-%d') + datetime.timedelta(weeks=-i)

        reservations = Reservation.query.filter_by(room_id=data['room_id'], date=date).all()
    ...
    reservation = Reservation(start_time=start_time, end_time=end_time, date=data['date'], user=user, room=room, resv_descr=data['resv_descr'], occupancy=data['occupancy'], repeat=data['repeat'], repeat_interval=data['repeat_interval'])
